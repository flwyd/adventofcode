...
class Solver {
  ...
  method evolve-steps($steps, :$print = False --> Int) {
    ...
    for ^$steps {
      ...
      $*ERR.say: "{%image.keys.elems} and {%image.values.sum} 1s points after step $_ with horizon $horizon" if $print;
...
class RunContext {
  ...
  method run-part(Solver:U $part) {
    ...
    $*ERR.say: "Running Day20 part $num on $!input-file expecting '$expected'";
    my $start = now;
